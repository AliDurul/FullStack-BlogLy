...
const LikeBtn = ({ blog, session }: ILikeCommentBtnProps) => {
    ...
    return (
        <>
            <button disabled={isPending} onClick={handleClick} className={`size-10 rounded-full flex items-center justify-center ${isLiked ? 'bg-red/20 text-red' : 'bg-grey/80'}`}>
                {
                    isPending ? (
                        <div role="status">
                            <svg aria-hidden="true" className="w-4 h-4 text-gray-200 animate-spin dark:text-gray-600 fill-rose-900" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
