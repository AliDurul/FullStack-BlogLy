...
export default function CommentBtn({ blog }: { blog: ISingleBlog }) {
    ...
    return (
        <>
            <button onClick={() => setCommentsWrapper(prev => !prev)} className={`size-10 rounded-full flex items-center justify-center bg-grey/80`}>
                <i className={`fi fi-rr-comment-dots `} />
            </button>
            <p className='text-xl text-dark-grey'>{blog.activity.total_comments}</p>

            {
                commentsWrapper && <CommentsContainer />
            }
        </>
    )
}
